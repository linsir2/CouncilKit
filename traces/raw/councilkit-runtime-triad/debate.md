...
- objection: FastAPI alone cannot enforce separation of runtime vs. data state—it’s a transport layer; the real enforcement must happen upstream in `harness_runtime.py` and `langgraph` checkpointing logic. Without that, even perfect FastAPI contracts become leaky facades.
...
- confidence: high
- evidence_path: FastAPI’s `response_model` is a hard security and schema boundary—not decoration—but only works if response types are strictly projection models, not domain objects, CouncilKit’s README explicitly forbids 'a second semantic' and treats artifacts as 'projections', which aligns precisely with FastAPI’s output filtering discipline, The `harness_contract.py` and `session_spec.py` already define explicit turn contracts; FastAPI should serialize those *as-is*, not leak underlying `llama-index` docstores or `langgraph` checkpointers
- needs_verification: Whether `src/councilkit/harness_runtime.py` currently exposes any data-store references to HTTP handlers, If `langgraph` checkpointers serialize full LLM state or only deterministic turn metadata, Whether `llama-index` ingestion paths write directly to global indexes or isolate per-session stores
...
- confidence: high
- evidence_path: LlamaIndex’s core identity is 'LLM over your data', not general agent orchestration [E2] [E3], Its value lies in拆able boundaries: retriever ≠ synthesizer ≠ query engine [E5] [E6] [E9], Settings is global default—not request-scoped state—so it cannot absorb CouncilKit’s per-session or per-turn data context [E10]
- needs_verification: Whether `harness_ingest.py` currently uses `VectorStoreIndex` or raw `Document` ingestion, Whether `harness_runtime.py` defines or validates any retrieval contract schema, Whether `session_spec.py` encodes retrieval constraints (e.g., metadata filters, top-k)
...
- confidence: high
- evidence_path: FastAPI’s `response_model` and type-driven validation are designed to filter and constrain output—not to mediate state ownership or mutability, Current `main.py` and `src/councilkit/runtime.py` show no use of `response_model` for session trace serialization, risking implicit leakage of internal objects, The `harness_runtime.py` file lacks explicit Pydantic models for turn artifacts, meaning FastAPI currently has no typed boundary to enforce
- needs_verification: Whether `src/councilkit/harness_runtime.py` already defines or imports Pydantic models for session/turn artifacts, Whether any existing route handler in `main.py` or `src/councilkit/runtime.py` directly injects or returns LlamaIndex or LangGraph primitives
...
- confidence: high
- evidence_path: CouncilKit’s current `harness_runtime.py` shows no `StateGraph`, `checkpointer`, or `interrupt()` usage—so adding durable execution without refactoring risks silent replay corruption., LangGraph v1 requires explicit reducers for any state key updated by multiple nodes; CouncilKit’s append-only turn logs currently lack merge semantics, making concurrent retrieval + execution updates unsafe., Official LangGraph docs state: ‘checkpointing is not a feature—it’s the foundation of recovery, replay, and thread continuity’ [E5]; CouncilKit has no checkpointer integration today.
- needs_verification: Whether `harness_runtime.py` can be incrementally refactored to delegate turn orchestration to a `StateGraph` without breaking existing `session_spec.py` contracts., If `src/councilkit/harness_ingest.py` already satisfies LlamaIndex’s ingestion contract requirements (e.g., `StorageContext` isolation, deterministic `Node` IDs) for reproducible retrieval., Whether FastAPI route handlers in `cli.py` or future API surface already expose raw `State` or `Index` objects—even indirectly—via dependency injection.
...
- confidence: high
- evidence_path: LlamaIndex’s core design assumes data boundaries are per-request or per-workflow, not per-runtime-session [E5][E6], CouncilKit’s current `harness_ingest.py` lacks `StorageContext` usage or retriever composition, indicating no existing data pipeline contract [E8][E9], `Settings` is global-default only—not request-scoped—so using it for retrieval config would leak state across turns [E10]
- needs_verification: Whether `harness_ingest.py` can be refactored to emit `Document`/`Node` objects compatible with `VectorStoreIndex` without leaking mutable state, If `RetrievalRequest` schema can be defined to include citation scope, reranking policy, and synthesizer selection without exposing LlamaIndex internals

## Synthesis Delta

### Coordinator Summary
All three skills converge on a core architectural tension: CouncilKit must add stateful execution (via LangGraph) and data-aware retrieval (via LlamaIndex) without eroding its foundational guarantees—replayability, explicit turn contracts, and the strict separation of runtime state (what happened, when, and who said it) from data state (what was retrieved, how, and from where). FastAPI reinforces that the HTTP layer must project only immutable, versioned artifacts—not expose or mediate underlying state. However, disagreement persists on *where enforcement originates*: FastAPI and LangGraph both object that current code lacks upstream state discipline (no `StateGraph`, no projection models, no `StorageContext`), making transport- or graph-layer boundaries leaky by default.

### Final Decision
Enforce the retrieval/data pipeline boundary as the non-negotiable separation point—requiring per-session, isolated `StorageContext` and `VectorStoreIndex` instances, with retrieval results explicitly serialized into turn artifacts via versioned schemas—because this is the only boundary whose collapse directly and irreversibly corrupts replay provenance (e.g., 'what was cited' vs. 'what was asserted').
...
- FastAPI (high): FastAPI alone cannot enforce separation of runtime vs. data state—it’s a transport layer; without upstream discipline in `harness_runtime.py` and LangGraph checkpointing, even perfect FastAPI contracts become leaky facades.
