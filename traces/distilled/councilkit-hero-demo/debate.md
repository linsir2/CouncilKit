...
- confidence: high
- evidence_path: main.py contains a single FastAPI app with no APIRouter decomposition yet, src/councilkit/runtime.py shows synchronous session execution without async I/O dependencies
...
- confidence: high
- evidence_path: The README explicitly states runtime is 'intentionally single-process, single-coordinator, and append-only' — this matches the manual-first constraint, SKILL.md files are declared the 'only semantic source', so manual execution surfaces gaps before automation hides them
...
- confidence: high
- evidence_path: CLI entrypoint `src/councilkit/cli.py` already exists and routes to `harness.py`, `runtime.py` contains turn logic but lacks enforced pause/inspect points
